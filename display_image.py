...
def get_hex_from_image(image,aspect_ratio):
    #abc = Image.open(rainbow)
    pix = image.load()
...
def get_rgb_from_image(image,aspect_ratio):
    ...
    result = [[pix[x,y] for x in range(aspect_ratio[0])] for y in range(aspect_ratio[1])]

    #print(result)
    return result

def get_rgb_from_image2(image,aspect_ratio):
    #abc = Image.open(rainbow)
    pix = image.load()
...
array_2d= turn_to_2d(aspect_ratio,array_LED)
color_strip = get_rgb_from_image(rainbow,aspect_ratio)
print(type(color_strip[0][0]))
while True:                              # Loop forever

    for row in range(len(array_2d)):
        for column in range(len(array_2d[row])):
            #r,g,b =get_rgb(color_strip[row][column])
            strip.setPixelColor(array_2d[row][column], color_strip[row][column][0],color_strip[row][column][1],color_strip[row][column][2])
    strip.show()
